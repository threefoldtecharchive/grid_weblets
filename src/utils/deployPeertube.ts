...
export default async function deployPeertube(data: VM, profile: IProfile) {
  ...
  // get the info
  const peertubeInfo = await getPeertubeInfo(client, name + "_postgres_vms");
  const peertubeYggIp = peertubeInfo[0]["yggIP"];

  // deploy the gateway
  await deployPrefixGateway(client, name, peertubeYggIp);
  // return the info
  const gatewayInfo = await getGatewayInfo(client, name + "_gateway");
  ...
  // // inject the domain name in the ui
  // const deploymentsInfo = JSON.stringify({});
  // return deploymentsInfo;
  // input [name]
  // output [name, ip, domain, ...]
}
