...
function _deployMatterMost(profile: IProfile, mattermost: Mattermost) {
  const { MachineModel, MachinesModel, generateString } = window.configs.grid3_client;

  const { name, username, password, server, domain, port, nodeId, cpu, memory, disks, publicIp, smtpPassword } =
    mattermost;
